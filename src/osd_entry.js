...
const DEFAULT_SETTINGS = DEFAULT_OSD_SETTINGS;
const MASTER_ICON =
...
function normalizeSettings(value = {}) {
  const settings = fromOsdSettings(value);
  return {
    ...settings,
    opacity: clampOsdOpacity(settings.opacity),
    scale: clampOsdScale(settings.scale),
  };
}
...
function createTargetIcon(option = {}) {
  if (option.icon_data) {
    ...
    image.src =
      source.startsWith("data:") || source.startsWith("assets/") ? source : `data:image/png;base64,${source}`;
    return image;
  }
...
async function refreshAudioSnapshot(force = false) {
  ...
  refreshInFlight = Promise.allSettled([
    ...
    .then(([nextSessions, nextFocus, nextPlayback, nextRecording]) => {
      if (nextSessions.status === "fulfilled" && Array.isArray(nextSessions.value))
        sessions = nextSessions.value;
      if (nextFocus.status === "fulfilled") focusedSession = nextFocus.value ?? null;
      if (nextPlayback.status === "fulfilled" && Array.isArray(nextPlayback.value))
        playbackDevices = nextPlayback.value;
      if (nextRecording.status === "fulfilled" && Array.isArray(nextRecording.value))
        recordingDevices = nextRecording.value;
...
async function start() {
  ...
  await Promise.all([
    listen("osd_settings_update", (event) => {
      const payload = parsePayload(event?.payload);
      if (!payload || typeof payload !== "object") return;
      const labelModeChanged =
        settings.showBindingName !== Boolean(payload.show_binding_name ?? payload.showBindingName ?? false);
