...
export function createVirtualAudioSettingsController({ invoke, dom, i18n, showAlert, renderSelectDropdown }) {
  ...
  function renderMeter(element, value) {
    if (!element) return;
    const percent = virtualAudioMeterPercent(value);
  ...
  function render() {
    const view = virtualAudioViewForStatus(status);
    ...
    setText(d.virtualAudioDiagnosticsCounts, t("virtualAudio.streamDiagnostics", {
    ...
    renderMeter(d.virtualAudioLimiterMeter, Math.min(1, status.limiter_reduction_db / 12));
    renderInputDevices();
  }

  async function refreshStatus() {
    try {
      status = normalizeVirtualAudioStatus(await invoke("get_virtual_audio_status"));
    } catch (error) {
  ...
  function startMeterPolling() {
    ...
    meterTimer = window.setInterval(async () => {
      if (!active || busy || meterRefreshPending || status.install_state !== "ready") return;
  ...
  function setActive(nextActive) {
    ...
    startMeterPolling();
    return loaded ? refreshStatus() : load();
  }
  ...
  function bindUi() {
    ...
    d.virtualAudioRefresh?.addEventListener("click", () => refreshStatus());
