...
const [source, html, domRefs, settingsSource, css, english, commandsSource, runtimeSource] =
  await Promise.all([
...
} = await import("../src/features/settings/virtual_audio.js");
      ...
      dom: {
        ...
        virtualAudioInstall: {
          ...
          addEventListener(type, listener) {
            clickListeners.set(type, listener);
          },
        },
      },
...
assert.match(
  html,
  /data-settings-section="virtual-audio"/,
  "Virtual Audio should have a dedicated settings navigation item",
);
assert.match(
  html,
  /data-settings-panel="virtual-audio"/,
  "Virtual Audio should have a dedicated settings panel",
);
assert.ok(
  html.indexOf('data-settings-section="appearance"') <
    html.indexOf('data-settings-section="virtual-audio"') &&
    html.indexOf('data-settings-section="virtual-audio"') <
      html.indexOf('data-settings-section="maintenance"'),
  "Virtual Audio should appear below Appearance and above Maintenance",
);
assert.doesNotMatch(
  html,
  /class="virtual-audio-intro"/,
  "Virtual Audio should not add a mismatched page title or description",
);
assert.match(
  html,
  /data-virtual-audio-state="loading"/,
  "Virtual Audio should start in a neutral loading state",
);
assert.match(
  html,
  /data-virtual-audio-view="loading"/,
  "Virtual Audio should show progress while its first status request runs",
);
assert.doesNotMatch(
  html,
  /id="virtual-audio-refresh"/,
  "live status polling should not also expose an unexplained refresh button",
);
assert.doesNotMatch(
  html,
  /id="virtual-audio-diagnostics-counts"/,
  "internal stream counters should not clutter component health",
);
for (const id of [
  ...
  const domName = id
    ...
    .replace(/^virtualAudio/, "virtualAudio");
...
assert.match(
  ...
  "microphone options should refresh the shared dropdown",
);
assert.match(
  source,
  /virtualAudioUpdateNotice\?\.classList\.toggle\("hidden", !status\.service_update_available\)/,
  "the update notice should follow backend service comparison",
);
assert.match(source, /setInterval[\s\S]*?250/, "ready-state meters should refresh live");
assert.match(
  source,
  /virtualAudioStatusRefreshInterval\(status\)/,
  "non-ready health states should continue refreshing at a throttled interval",
);
assert.doesNotMatch(
  source,
  /if\s*\(\s*status\.install_state !== "ready"\s*\)\s*return/,
  "startup health polling should not freeze on a transient service error",
);
assert.match(
  source,
  /settleStatusAfterAction\(command\)/,
  "setup actions should wait for a fresh component-health result before leaving the installing view",
);
assert.match(
  source,
  /force:\s*true,\s*renderResult:\s*false/,
  "post-install health checks should bypass the backend component cache without flashing stale state",
);
assert.match(css, /\.virtual-audio-ready-layout/, "Virtual Audio should have dedicated layout styles");
assert.match(
  css,
  /\.virtual-audio-ready-layout\s*\{[^}]*flex:\s*1 1 auto;[^}]*min-height:\s*0;[^}]*grid-template-rows:\s*auto minmax\(min-content, 1fr\)/s,
  "the ready layout should fill the settings panel and give remaining height to component health",
);
assert.match(
  css,
  /\.virtual-audio-health-card > \.virtual-audio-button-row\s*\{[^}]*margin-top:\s*auto/s,
  "component health actions should stay anchored to the bottom of the stretched card",
);
assert.match(
  css,
  /\.virtual-audio-section\.active\s*\{[^}]*overflow:\s*visible/s,
  "the compact Virtual Audio panel should not add a nested scrollbar",
);
assert.match(css, /--meter-level/, "meters should expose their current level to CSS");
assert.match(
  commandsSource,
  /pub async fn get_virtual_audio_status/,
  "status polling should run away from the UI thread",
);
assert.match(
  commandsSource,
  /force:\s*Option<bool>/,
  "status polling should support explicit cache bypass after setup changes",
);
assert.match(
  commandsSource,
  /service_update_available/,
  "status should report when the bundled service differs from the installed service",
);
assert.doesNotMatch(commandsSource, /refresh_if_due/, "status polling should not trigger route refreshes");
assert.match(
  runtimeSource,
  /lifecycle:\s*Mutex<\(\)>/,
  "route start, refresh, and stop should be serialized",
);
assert.match(
  runtimeSource,
  /impl Drop for ActiveRoute/,
  "abandoned routes should always release their workers and audio pipe",
);
